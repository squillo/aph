...
**The v0.3 delta** (`spec/aph-0.3-draft.md`, DRAFT — opened 2026-08-31) adds NO envelope members and NO codes: RFC 0009 names the two passing status dispositions (`StatusAbsent`/`StatusLive`, closed set, evidence rule with instant + status-list id; emission optional) and RFC 0010 makes vocabulary citation digests SRI-style (SHA-256 over fetched artifact bytes, in the citation, never inside the artifact — superseding v0.1's "the bundle carries" clause, which a stranger cannot byte-check). The blessed guardrails citation is `aph_guardrails@0.1.0`, digest `sha256-iTJRGKxirtZFmyDmoGPdPyRzIevrjVddmWhMnCa6Od0=` over `snapp/aph_guardrails@0.1.0.json` (184 labels — not 182).
