...
//! readership.
//!
//! ⚠ EXPERIMENTAL, 0.2-DRAFT WIRE. RFC 0008 is Accepted onto the v0.2
//! line and implemented here: the WIRE types live in `aph-core`
...
pub enum EnvelopeSealError {
  ...
  /// A protocol refusal with its 0.2-draft code: `APH_E021` (the seal
...
pub fn seal_into_envelope(
  ...
  if envelope.aph_version != "0.2" {
    return std::result::Result::Err(EnvelopeSealError::WireUndeclared(std::format!(
      "refusing to seal into an aphVersion `{}` envelope: `sealedPayload` is        declared from aphVersion 0.2 (spec/aph-0.2-draft.md), and minting it        earlier produces a wire every conformant verifier refuses",
...
mod tests {
  ...
  fn envelope_unseal_failures_carry_their_draft_codes() {
    // The two runtime refusals map to the 0.2-draft codes the spec delta
